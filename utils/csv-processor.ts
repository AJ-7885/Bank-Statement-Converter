...
export function detectSeparator(csvText: string): string {
  ...
  console.log(`Separator detection: semicolons=${semicolonCount}, commas=${commaCount}`)
...
export function convertToCSV(data: any[]): string {
  ...
  data.forEach((row) => {
    const csvRow = [
      ...
      row.referenceNo || "",
      row.qty || "",
      row.debitUnit !== null && row.debitUnit !== undefined ? row.debitUnit.toFixed(2) : "",
      row.creditUnit !== null && row.creditUnit !== undefined ? row.creditUnit.toFixed(2) : "",
    ]
    rows.push(csvRow)
  })
