...
import { ProcessingAnalysis } from "../../components/processing-analysis";
...
export default function BankStatementConverter() {
  ...
  const initializeSteps = (bankKey: string): ProcessingStep[] => {
    ...
    const bankSpecificSteps = {
      ...
      amex: [
        ...
        {
          step: 4,
          description: "Extract description from columns G + H",
          status: "pending" as const,
        },
        {
          step: 5,
        ...
        {
          step: 6,
          description: "Apply lenient validation (process ALL rows)",
          status: "pending" as const,
        },
        {
          step: 7,
  ...
  const runDataAnalysis = async () => {
    if (selectedBank === "amex") {
      // Run the analysis script for American Express
      console.log("Running American Express data analysis...");
      // The analysis script will run automatically when imported
    }
  };
      ...
      {inputRowCount > 0 && conversionResult && (
        <div className="mb-8">
          <ProcessingAnalysis
            ...
            expectedRows={inputRowCount}
            bankName={bankConfigs[selectedBank]?.name || selectedBank}
          />
        </div>
      )}
