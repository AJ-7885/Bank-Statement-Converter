...
export function processAmexData(data: string[][], config: BankConfig): StandardizedTransaction[] {
      ...
      if (!description) {
        ...
        // Try combining multiple description fields
          ...
          for (let j = 1; j < Math.min(row.length, 8); j++) {
            ...
            if (part && part !== "" && !part.match(/^\d+[.,]\d+$/) && !part.match(/^-?\d+[.,]\d+$/)) {
              descParts.push(part)
            }
          }
      ...
      if (row.length > 4) {
        ...
        if (amountStr && amountStr !== "" && amountStr !== "0" && amountStr !== "0,00") {
          ...
          if (parsedAmount !== null && parsedAmount > 0) {
            ...
            if (i < 5) {
              console.log(`Row ${i + 1}: Amount "${amountStr}" -> D-Unit: ${debitAmount}, C-Unit: ${creditAmount}`)
            }
          }
        }
      }
