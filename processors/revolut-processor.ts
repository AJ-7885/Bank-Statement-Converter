...
export function processRevolutData(data: string[][], config: BankConfig): StandardizedTransaction[] {
      ...
      if (amountStr) {
        ...
        if (parsedAmount !== null) {
          ...
          if (amountStr.includes("-") || parsedAmount < 0) {
            ...
            console.log(`Row ${i + 1}: Negative amount -> D-Unit: ${debitAmount}`)
          } else {
            creditAmount = parsedAmount
            console.log(`Row ${i + 1}: Positive amount -> C-Unit: ${creditAmount}`)
          }
        }
      }
