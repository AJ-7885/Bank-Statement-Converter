    ...
    try {
      ...
      if (row.length > 15) {
        ...
        if (
          ...
          console.log(
            `Row ${
              i + 1
            }: Column P[15] (D-Unit) = "${debitStr}" -> ${debitAmount}`
          );
        }
      }
      ...
      if (row.length > 16) {
        ...
        if (
          ...
          console.log(
            `Row ${
              i + 1
            }: Column Q[16] (C-Unit) = "${creditStr}" -> ${creditAmount}`
          );
        }
      }
      ...
      if (debitAmount === null && creditAmount === null && row.length > 10) {
        console.log(
          `Row ${
            i + 1
          }: Columns P/Q not found, searching last columns for amounts`
        );
            ...
            if (parsedAmount !== null && parsedAmount > 0) {
              if (creditAmount === null) {
                ...
                console.log(
                  `Row ${
                    i + 1
                  }: Found amount in column ${colIndex} -> C-Unit: ${parsedAmount}`
                );
                ...
                console.log(
                  `Row ${
                    i + 1
                  }: Found amount in column ${colIndex} -> D-Unit: ${parsedAmount}`
                );
                break;
              }
            }
      ...
      if (
        ...
        console.log(
          `Row ${
            i + 1
          }: ✅ Added transaction - Date: ${finalDate}, Desc: "${description.substring(
            0,
            30
          )}...", D-Unit: ${debitAmount}, C-Unit: ${creditAmount}`
        );
      } else {
