  ...
  for (let i = 0; i < data.length; i++) {
    ...
    try {
      ...
      if (row.length > 15) {
        ...
        if (debitStr && debitStr !== "" && debitStr !== "0" && debitStr !== "0,00" && debitStr !== "NULL") {
          debitAmount = parseEuropeanNumber(debitStr)
          console.log(`Row ${i + 1}: Column P[15] (D-Unit) = "${debitStr}" -> ${debitAmount}`)
        }
      }
      ...
      if (row.length > 16) {
        ...
        if (creditStr && creditStr !== "" && creditStr !== "0" && creditStr !== "0,00" && creditStr !== "NULL") {
          creditAmount = parseEuropeanNumber(creditStr)
          console.log(`Row ${i + 1}: Column Q[16] (C-Unit) = "${creditStr}" -> ${creditAmount}`)
        }
      }
      ...
      if (debitAmount === null && creditAmount === null && row.length > 10) {
        console.log(`Row ${i + 1}: Columns P/Q not found, searching last columns for amounts`)

        // Look for amounts in the last few columns
        for (let colIndex = row.length - 1; colIndex >= Math.max(10, row.length - 5); colIndex--) {
          const cellValue = row[colIndex]?.trim() || ""
          ...
          if (cellValue && cellValue !== "" && cellValue !== "0" && cellValue !== "0,00") {
            ...
            if (parsedAmount !== null && parsedAmount > 0) {
              if (creditAmount === null) {
                ...
                console.log(`Row ${i + 1}: Found amount in column ${colIndex} -> C-Unit: ${parsedAmount}`)
              } else if (debitAmount === null) {
                debitAmount = parsedAmount
                console.log(`Row ${i + 1}: Found amount in column ${colIndex} -> D-Unit: ${parsedAmount}`)
                break
              }
            }
          }
      ...
      if (finalDate || description || debitAmount !== null || creditAmount !== null) {
        results.push({
          date: finalDate,
          category: "",
